...
#         cv2.rectangle(img, (x,y), (x+w, y+h),  color['green'], 2)  #uncomment if you want to see face boundary
        ...
        nose_cords = ((2*x+w)//2,(2*y+h)//2) # it is center
...
def keyboard_events(nose_cords, cords, cmd):
    try:
        [(x1,y1), (x2,y2)] = cords
        xc, yc = nose_cords # xc and yc are the centre of nose
    ...
    if xc < x1:
        ...
        cmd = "down"  # See once this up and down commands
    if cmd:  # None pe nhi chalega
        print("Detected movement: ", cmd,  "\n")
        keyboard.press_and_release(cmd) #ye sirf un programs ke liye chalega jiske liye wo apne aap seedha jaye(without any pressing key like subway surfer and temple run)
    return img,cmd
def reset_press_flag(nose_cords, cords,cmd): # cords means reference(blue color)
    try:
        [(x1,y1), (x2,y2)] = cords
        xc, yc = nose_cords
    except: 
        return True,cmd
    if x1<xc<x2 and y1<yc<y2:  # Initially 
        return True,None
    return False,cmd
...
while True:
    ...
    if press_flag and len(nose_cords): # agar person ki nose reference mein nhi hui toh keyboard events fire nhi hoge 
